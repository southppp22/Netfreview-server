    ...
    async findThisVidAndUserReview(video, user) {
        ...
        if (rawVideoList.length) {
            rawVideoList.map(async (oneVideo) => {
                videoList.push(Object.assign(Object.assign({}, oneVideo), { likeCount: await this.likeRepository.count({ review: oneVideo }), isLike: await this.likeRepository.count({ user, review: oneVideo }), reviewId: oneVideo.id }));
            });
        }
        else {
